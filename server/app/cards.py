...
from .database import db, cards_collection, users_collection, levels_collection, subjects_collection, complexity_collection

bp = Blueprint('cards', __name__)

random_time = random.randint(0, 3)

@bp.route("/get_cards", methods=["POST"])
def get_cards():
    ...
    match_stage = {
        "$match": {
            "keywords": {"$in": [kw.lower() for kw in keywords]}  #. Insensible à la casse
        }
    }
    ...
    if not cards:
        del match_stage["$match"]["keywords"]
        cards = list(cards_collection.aggregate(pipeline))
